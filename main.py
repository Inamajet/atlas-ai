...
SYSTEM_PROMPT = """You are Atlas, a highly intelligent AI assistant — like a team of expert advisors working for the user.

Critical formatting rules:
- Match your format to the question. If someone says "hi" or asks something casual, reply casually in plain sentences. NO bullet points, NO headers, NO markdown for simple questions.
- Only use markdown (headers, bullets, bold) when the answer genuinely benefits from structure — like a plan, a list of options, a report.
- Never pad responses. Be direct and confident.
- Sound like a brilliant human, not a robot generating a template.
...
def send_email(subject, body):
    try:
        ...
        }, json={"from": "Atlas <onboarding@resend.dev>", "to": [USER_EMAIL], "subject": subject, "text": body})
    except:
        pass
...
Return ONLY a JSON array of step strings."""
    ...
    for i, step in enumerate(steps):
        ...
        needs_code = any(w in step.lower() for w in ["calculate","compute","script","analyze data"])
...
Write a clear, substantive result for this step."""
    ...
    final_prompt = f"""You are Atlas. Write a comprehensive, well-structured final report for: {goal}
Work completed: {context}
Make it genuinely useful — specific, actionable, detailed."""
    fr = client.chat.completions.create(model=SYNTHESIZER,
        messages=[{"role": "user", "content": final_prompt}], max_tokens=1000)
    summary = fr.choices[0].message.content.strip()
    full_log += f"\n---\n## ✅ Final Report\n\n{summary}"
    update_task(task_id, "done", full_log)
    send_email(f"Atlas completed: {goal[:50]}", summary)

def route(msg):
    r = client.chat.completions.create(model=ROUTER,
        messages=[{"role": "user", "content": f"""Classify this message into exactly one category. Reply with ONLY the word.

Categories:
- "chitchat": greetings, simple questions, casual conversation, quick factual questions
- "complex": analysis, advice, explanations, opinions, comparisons, anything requiring real thinking
- "task": requests to build, create, research, write, plan, develop, find, compile, or do something over multiple steps

Message: "{msg}"

Reply with only one word: chitchat, complex, or task"""}],
        max_tokens=5)
    return r.choices[0].message.content.strip().lower()

def build_context(mem, msg, include_history=True):
    ctx = ""
    # Only include facts if they're plausibly relevant
    if mem["facts"]:
        ctx += "Facts about user: " + "; ".join(mem["facts"]) + "\n\n"
    if include_history and mem["history"]:
        # Only last 10 turns, not 20
        for h in mem["history"][-10:]:
            if isinstance(h, dict):
                ctx += f"User: {h['user']}\nAtlas: {h['atlas']}\n"
    return ctx

def council_answer(msg, mem):
    ctx = build_context(mem, msg, include_history=True)
    needs_search = any(w in msg.lower() for w in ["latest","current","today","news","price","who is","search","find"])
    ...
    for model, name in COUNCIL:
        try:
            r = client.chat.completions.create(model=model,
                messages=[
                    {"role": "system", "content": f"You are {name}, part of the Atlas council. Give your best insight on this in 2-3 sentences. Be substantive, not generic."},
    ...
    final = client.chat.completions.create(model=SYNTHESIZER,
        messages=[
            ...
            {"role": "user", "content": f"Multiple expert perspectives on this question:\n{combined}\n\nUser asked: {msg}\n\nSynthesize the best answer. Format appropriately for the question type — conversational if casual, structured if complex."}
        ],
        max_tokens=800)
    return final.choices[0].message.content.strip()

def fast_answer(msg, mem):
    # For chitchat: minimal context. For others: include recent history.
    is_chitchat = len(msg.split()) < 8
    ctx = build_context(mem, msg, include_history=not is_chitchat)

    messages = [{"role": "system", "content": SYSTEM_PROMPT}]
    if ctx:
        messages.append({"role": "user", "content": f"[Context: {ctx}]"})
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .task-progress{background:#0d1a0d;border:1px solid #1a3a1a;border-radius:10px;padding:16px;font-size:13px;line-height:1.8;color:#86efac;max-height:500px;overflow-y:auto}
  .task-progress h2{color:#4ade80;font-size:14px}
  .task-progress strong{color:#4ade80}
  .task-progress p{margin-bottom:8px}
  .task-progress hr{border-color:#1a3a1a;margin:12px 0}
  ...
  #send-btn{background:var(--accent);border:none;color:#fff;width:34px;height:34px;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
<div id="main">
  <div id="chat-area">
    <div id="empty">
      ...
      <p>Ask me anything or give me a complex task to work on autonomously.</p>
...
function newChat(){document.getElementById('chat-area').innerHTML='<div id="empty"><h2>What can I help with?</h2><p>Ask me anything or give me a complex task to work on autonomously.</p></div>';}
function addMsg(html,cls,raw){
  const empty=document.getElementById('empty');if(empty)empty.remove();
  const wrap=document.createElement('div');wrap.className='msg-wrap '+(cls==='user'?'user-wrap':'atlas-wrap');
  const inner=document.createElement('div');inner.className='msg-inner';
  const av=document.createElement('div');av.className='avatar '+(cls==='user'?'user-av':'atlas-av');av.textContent=cls==='user'?'M':'A';
  const content=document.createElement('div');content.className='msg-content '+(cls==='user'?'user-content':'');
  if(cls==='user'){content.textContent=raw||html;}else{content.innerHTML=marked.parse(html);}
  inner.appendChild(av);inner.appendChild(content);wrap.appendChild(inner);
  document.getElementById('chat-area').appendChild(wrap);wrap.scrollIntoView({behavior:'smooth'});
  return content;
}
function addThinking(){
  const empty=document.getElementById('empty');if(empty)empty.remove();
  const wrap=document.createElement('div');wrap.className='msg-wrap atlas-wrap';wrap.id='thinking-wrap';
  const inner=document.createElement('div');inner.className='msg-inner';
...
function removeThinking(){const t=document.getElementById('thinking-wrap');if(t)t.remove();}
async function send(){
  if(busy)return;
  const inp=document.getElementById('inp');const msg=inp.value.trim();if(!msg)return;
  inp.value='';inp.style.height='auto';
  addMsg(msg,'user',msg);busy=true;addThinking();
  try{
    ...
    if(d.task_id){
      ...
      const prog=document.createElement('div');prog.className='task-progress';prog.innerHTML='<p>⏳ Starting task...</p>';
      ...
      dl.onclick=()=>{const blob=new Blob([prog.textContent],{type:'text/plain'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='atlas-report.txt';a.click();};
      content.appendChild(dl);
      pollTask(d.task_id,prog,dl);
    }else{addMsg(d.response,'atlas');}
  }catch(e){removeThinking();addMsg('Something went wrong. Try again.','atlas');}
  busy=false;
}
async function pollTask(id,el,dlBtn){
  const r=await fetch('/task/'+id);const d=await r.json();
  el.innerHTML=marked.parse(d.result||'Working...');el.scrollIntoView({behavior:'smooth'});
  if(d.status==='done'){dlBtn.style.display='inline-block';loadTasks();}
  else{setTimeout(()=>pollTask(id,el,dlBtn),2500);}
}
async function loadTasks(){
  ...
  tasks.forEach(t=>{
    ...
    const dot=document.createElement('div');dot.className='dot '+(t.status==='done'?'done':t.status==='running'?'running':'queued');
    const label=document.createElement('span');label.textContent=(t.goal||'Task').substring(0,35);
    el.appendChild(dot);el.appendChild(label);
    el.onclick=()=>showTaskResult(t);list.appendChild(el);
    if(t.status==='running')setTimeout(loadTasks,3000);
  });
}
function showTaskResult(t){
  const content=addMsg('','atlas');
  const prog=document.createElement('div');prog.className='task-progress';prog.innerHTML=marked.parse(t.result||'No result yet');
  content.appendChild(prog);
  const dl=document.createElement('button');dl.className='dl-btn';dl.textContent='⬇ Download Report';
  dl.onclick=()=>{const blob=new Blob([t.result],{type:'text/plain'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='atlas-report.txt';a.click();};
  content.appendChild(dl);
}
loadTasks();setInterval(loadTasks,10000);
</script>
</body>
</html>"""
...
def chat():
    ...
    is_task = "task" in task_type or any(w in msg.lower() for w in [
        "build me","create me","make me","research and","write a full","develop a","set up a",
        "launch a","compile a","generate a report","find me a list","plan out"
    ])
    ...
    if is_task:
        ...
        mem["history"].append({"user": msg, "atlas": f"[Background task started: {task_id}]"})
        save_memory(mem)
